...
export async function GET(
  ...
  const [assets, versions] = await Promise.all([
    assetsRepo.getAssetsByProjectId(projectId, userId),
    versionsRepo.getVersionNodesByProjectId(projectId, userId),
  ]);
  return NextResponse.json({ project, assets, versions });
}
